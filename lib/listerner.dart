...
class SliverReorderableListState extends State<SliverReorderableList>
  ...
  void _registerItem(_ReorderableItemState item) {
    if (_dragInfo != null && _items[item.index] != item) {
      item.updateForGap(_dragInfo!.index, _dragInfo!.index,
          _dragInfo!.itemExtent, false, _reverse, _dragInfo);
    }
  ...
  Drag? _dragStart(Offset position) {
    ...
    for (final _ReorderableItemState childItem in _items.values) {
      ...
      childItem.updateForGap(_insertIndex!, _insertIndex!,
          _dragInfo!.itemExtent, false, _reverse, _dragInfo);
    }
    return _dragInfo;
  }
  ...
  void _dragUpdate(_DragInfo item, Offset position, Offset delta) {
    setState(() {
      ...
      _dragUpdateItems();
      print((item.initialPosition.dy - item.dragPosition.dy).abs() > 50);
      print((item.initialPosition.dy - item.dragPosition.dy).abs().toString() +
          'delta dx');
      print(position.dy.toString() + 'position dy');
      // print(position.dx.toString() + 'position dx');
      // print(item.dragOffset.toString() + 'drag g');
      print(item.dragPosition.dy.toString() + 'posi g');
      print(item.initialPosition.dy.toString() + 'posi g');
      // if ((item.initialPosition.dy - item.dragPosition.dy).abs() < 50) return;
      print('fdsfvsdfvsdfvs');
      print((item.initialPosition.dy - item.dragPosition.dy).abs() > 50);
      // _resetItemGap();
      // widget.onReorder.call(item.index + 1, 0);
  ...
  void _handleScrollableAutoScrolled() {
    ...
    _dragUpdateItems();
    // if ((_dragInfo!.initialPosition.dy - _dragInfo!.dragPosition.dy).abs() > 50)
    //   return;
    print('fdsfvsdfvsdfvs');
    print((_dragInfo!.initialPosition.dy - _dragInfo!.dragPosition.dy).abs() >
        50);
  ...
  void _dragUpdateItems() {
    ...
    if (newIndex != _insertIndex) {
      ...
      for (final _ReorderableItemState item in _items.values) {
        ...
        item.updateForGap(
            _dragIndex!, newIndex, gapExtent, true, _reverse, _dragInfo);
      }
    }
  }
...
class _ReorderableItemState extends State<_ReorderableItem> {
  ...
  Widget build(BuildContext context) {
    if (_dragging) {
      // _listState._unregisterItem(widget.dragInfo!.index, this);
      final isOutside = ((widget.dragInfo!.initialPosition.dy -
                  widget.dragInfo!.dragPosition.dy)
              .abs() >
          80);
      ...
      return AnimatedSize(
        ...
        child:
            isOutside ? const SizedBox.shrink() : SizedBox.fromSize(size: size),
      );
  ...
  void updateForGap(int dragIndex, int gapIndex, double gapExtent, bool animate,
      bool reverse, _DragInfo? _dragInfo) {
    ...
    if ((_dragInfo.initialPosition.dy - _dragInfo.dragPosition.dy).abs() > 80) {
      resetGap();
